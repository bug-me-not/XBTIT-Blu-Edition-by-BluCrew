...
if (isset ($_GET["mid"]) && preg_match('/^[0-9]+$/',$_GET["mid"])) {
  ...
  echo "<HTML><HEAD>\n";
  echo " <TITLE>".$movie->title().' ('.$movie->year().") [IMDBPHP2 v".$movie->version."]</TITLE>\n";
  ...
  echo "[IMDBPHP2 v".$movie->version."] Movie Details for '" . $movie->title()."' (".$movie->year().")";
...
if ($CURUSER['admin_access'] == 'yes'){
	$title = preg_replace('/\:/', '', $movie->title(), -1);
...
if ($imdb = "" || !$imdb)	{
	...
	$streaming .= "No IMDB Data<br /><br />";
}else{

$query = do_sqlquery("SELECT * FROM {$TABLE_PREFIX}stream_servers WHERE sid != 1 ORDER BY sid ASC");
$server_list = "";	
while ($servers = $query->fetch_assoc()){	
	$server_list .= "<option value=\"{$servers['sid']}\">{$servers['server_url']}</option>";
}
...
$addform = "
		<form action=\"../index.php?page=modules&module=video_player&addstream=1\" method=\"post\">
<select name=\"server\">
{$server_list}
</select><br /><br />
<input type=\"title\" name=\"title\" value=\"{$title}\" size=\"{$titlesize}\" readonly>.<input type=\"year\" name=\"year\" value=\"{$year}\" size=\"4\" readonly>.
<select name=\"quality\">
  <option value=\"1\">480p</option>
  <option value=\"2\" {$sel720}>720p</option>
  <option value=\"3\" {$sel1080}>1080p</option>
...
<input type=\"hidden\" name=\"imdb\" value=\"{$movieid}\">
		
		<input type=\"submit\" value=\"Add\"></form><br /><br />";
		
  echo '<TR><TD><B>Add Stream:</B></TD><TD>'.$addform."</TD></TR>\n";
  }
}
  # Keywords
