...
class Sypex_Dumper {
	...
	function connect($host = null, $port = null, $user = null, $pass = null){
		...
		if(mysql_connect($this->CFG['my_host'] . ($this->CFG['my_host']{0} != ':' ? ":{$this->CFG['my_port']}" : ''),  $this->CFG['my_user'], $this->CFG['my_pass'])) {
			quickQuery('SET NAMES utf8') or sxd_my_error();
			define('V_MYSQL', sxd_ver2int(mysql_get_server_info()));
		}
		else {
			define('V_MYSQL', 0);
			$this->error = "sxd.actions.tab_connects();alert('{" . sqlesc(sql_error()) . "');";
		}
		$this->try = false;
	...
	function ajax($req){
		...
		switch($req['act']){
			...
			case 'exit':
				setcookie('sxd', '', 0);
				$res = "this.location.href = '" . sqlesc($this->CFG['exitURL']) . "'";
				break;
		}
		echo $res;
	}
	...
	function addDb($req){
        $r = do_sqlquery('CREATE DATABASE `' . sqlesc($req['name']) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : ''));
        if($r)
	...
	function runRestoreJob($continue = false){
		...
		//$enable_index = array();
		if(V_MYSQL > 40014) {
			quickQuery("SET UNIQUE_CHECKS=0");
			quickQuery("SET FOREIGN_KEY_CHECKS=0");
			if(V_MYSQL > 40101) quickQuery("SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'");
			if(V_MYSQL > 40111) quickQuery("SET SQL_NOTES=0");
		}
		$log_sql = false;
		$fields = '';
		$time_old = time();
			...
			switch($q{0}){
				...
				case 'I':
					if (preg_match('/^INSERT( INTO `(.+?)`) VALUES/', $q, $m)) {
						...
						//quickQuery("LOCK TABLES `{$tab}` WRITE") or die (sql_error());
						quickQuery("ALTER TABLE `{$tab}` DISABLE KEYS") or sxd_my_error();
						if(!empty($this->JOB['autoinc'])) quickQuery("ALTER TABLE `{$tab}` AUTO_INCREMENT = 1") or sxd_my_error();
						$ex = 1;
					}
					break;
				case 'C':
				...
				case '#': // Команды для дампера
					if (preg_match("/\#\t(TC|TD|VI|PR|FU|TR|EV)`(.+?)`(([^_]+?)_.+?)?$/", $q, $m)) {
						...
						elseif($m[1] == 'TD'){
							...
							$this->rtl[7] = 0;
							$this->rtl[8] = 0;
							quickQuery("DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sxd_my_error();
		...
		foreach($this->JOB['todo']['TA'] AS $tab){
			...
			quickQuery("ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sxd_my_error();
			$this->rtl[1] = time();
			$this->rtl[5] = $tab[1];
			fseek($this->fh_rtl, 0);
			fwrite($this->fh_rtl, implode("\t", $this->rtl));
	...
	function addBackupJob($job) {
		...
		$this->JOB['todo'] = $todo;
		$this->saveJob($this->JOB['job'], $this->JOB);
		$fcache = implode('|', array('#SXD20', V_SXD, V_MYSQL, V_PHP, date('Y.m.d H:i:s'), $this->JOB['db'], $this->JOB['charset'], $tabs, $rows, sqlesc($this->JOB['comment']))) . "\n";
	...
	function runBackupJob($continue = false){
		...
		quickQuery("SET SQL_QUOTE_SHOW_CREATE = 1");
		$types = array('VI' => 'View', 'PR' => 'Procedure', 'FU' => 'Function', 'TR' => 'Trigger', 'EV' => 'Event');
		$fcache = '';
		$writes = 0;

		quickQuery("SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') ."'") or sxd_my_error();
		$time_old = time();
		$exit_time = $time_old + $this->CFG['time_web'] - 1;
					...
					case 'TA':
			            ...
			            while($row = $r->fetch_row()) {
							...
							for($k = 0; $k < $fields; $k++){
								...
								elseif($notNum[$k]) {$row[$k] =  '\'' . sqlesc($row[$k]) . '\'';}
							}
							$fcache .= '(' . implode(',', $row) . "),\n";
							$this->rtl[7]++;
							$this->rtl[10]++;
	...
	function setNames($collation){
		...
		if($this->rtl[6] != $collation) {
			quickQuery('SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sxd_my_error();
	...
	function formatTree($tree, &$objects){
		...
		foreach($objects AS $type => $o){
			...
			if($type == 'TA') {
				...
				$obj .= "[{$row},0,'" . sqlesc($info[$type][0]) . "',1,1,1],";
				$row++;
				foreach($o['*'] AS $value){
					if(is_string($value)){
						if(count($o[$value]) > 1)	{
							...
							for($i = 0, $l = count($o[$value]); $i < $l; $i++){
								...
								$obj .= "[{$row},{$pid},'" . sqlesc($o[$value][$i][0]) . "',2,{$checked},{$o[$value][$i][2]}],";
								$row++;
							}
						}
						else {
				...
				$obj .= "[{$row},0,'" . sqlesc($info[$type][0]) . "',{$info[$type][1]},1,1],";
				$pid = $row++;
				$info[$type][1]++;
...
function sxd_error_handler($errno, $errmsg, $filename, $linenum, $vars){
    ...
    if($errno == 8 || $errno == 1024) {
    	if (!$SXD->fh_log && !$SXD->fh_rtl) echo isset($_POST['ajax']) ? "alert('" . sqlesc($str) . "');" : $str;
    	else {
