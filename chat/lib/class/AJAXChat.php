...
class AJAXChat {
	...
	function login() {
		...
		if($this->isUserOnline($userData['userID']) || $this->isUserNameInUse($userData['userName'])) {
			if($userData['userRole'] == '9' || $userData['userRole'] == '12' || $userData['userRole'] == AJAX_CHAT_BLURG || $userData['userRole'] == AJAX_CHAT_MODERATOR || $userData['userRole'] == AJAX_CHAT_ADMIN) {
		...
		if($userData['userRole'] != AJAX_CHAT_ADMIN && $userData['userRole'] != AJAX_CHAT_MODERATOR && $this->isUserBanned($userData['userName'], $userData['userID'], $_SERVER['REMOTE_ADDR'])) {
	...
	function isAllowedToSendPrivateMessage() {
		if($this->getConfig('allowPrivateMessages') || $this->getUserRole() == AJAX_CHAT_MODERATOR || $this->getUserRole() == AJAX_CHAT_ADMIN) {
			return true;
		}
		return false;
	}
	...
	function isAllowedToCreatePrivateChannel() {
		if($this->getConfig('allowPrivateChannels')) {
			switch($this->getUserRole()) {
				case AJAX_CHAT_VIP:
					return true;
				case AJAX_CHAT_TRUSTEE:
					return true;
				case AJAX_CHAT_BLURG:
					return true;
	...
	function replaceCustomText(&$text) {
			...
			$s = $text;
			$file = AJAX_CHAT_PATH."/../badwords.txt";
