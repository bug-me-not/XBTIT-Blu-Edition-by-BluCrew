...
var ajaxChat = {
	...
	getUserNodeString: function(userID, userName, userRole) {
		...
		if(this.userNodeString && userID === this.userID) {
			...
			// HDVinnie User Class Icons
			var userRoleIcon; 
			
			if(userRole == 12){
				var userRoleIcon = '<img class="rolicon" alt="" src="img/donor.gif" title="VIP" />';
			}
			else if(userRole == 0){
				// Guest
				var userRoleIcon = '<img alt="" class="rolicon" src="img/blank.png" title="Guest" />';
			}
			else if(userRole == 1){
				// User
				var userRoleIcon = '<img alt="" class="rolicon" src="img/blank.png" title="User" />';
			}
			else if(userRole == 2){
				// Superleech
				var userRoleIcon = '<img alt="" class="rolicon" src="img/leech.png" title="Superleech" />';
			}
			else if(userRole == 3){
				// Leecher
				var userRoleIcon = '<img alt="" class="rolicon" src="img/leech.png" title="Leecher" />';
			}
			else if(userRole == 4){
				// Recruit
				var userRoleIcon = '<img alt="" class="rolicon" src="img/recruit.png" title="Recruit" />';
			}
			else if(userRole == 5){
				// BluUser
				var userRoleIcon = '<img alt="" class="rolicon" src="img/bluuser.png" title="BluUser" />';
			}
			else if(userRole == 6){
				// BluWarrior
				var userRoleIcon = '<img alt="" class="rolicon" src="img/bluwarrior.png" title="BluWarrior" />';
			}
			else if(userRole == 7){
				// BluMaster
				var userRoleIcon = '<img alt="" class="rolicon" src="img/blumaster.png" title="BluMaster" />';
			}
			else if(userRole == 8){
				// BluAddict
				var userRoleIcon = '<img alt="" class="rolicon" src="img/bluaddict.png" title="BluAddict" />';
			}
			else if(userRole == 9){
				// BluJunkie
				var userRoleIcon = '<img alt="" class="rolicon" src="img/blujunkie.png" title="BluJunkie" />';
			}
			else if(userRole == 10){
				// BluGod
				var userRoleIcon = '<img alt="" class="rolicon" src="img/blugod.png" title="BluGod" />';
			}
			else if(userRole == 11){
				// Uploader
				var userRoleIcon = '<img alt="" class="rolicon" src="img/uploader.png" title="Uploader" />';
			}
			else if(userRole == 13){
				// Trustee
				var userRoleIcon = '<img alt="" class="rolicon" src="img/shield.png" title="Trustee" />';
			}
			else if(userRole == 14){
				// BluRG
				var userRoleIcon = '<img alt="" class="rolicon" src="img/blurg.png" title="BluRG" />';
			}
			else if(userRole == 15){
				// Moderator
				var userRoleIcon = '<img alt="" class="rolicon" src="img/mod.png" title="Moderator" />';
			}
			else if(userRole == 16){
				// Admin
				var userRoleIcon = '<img alt="" class="rolicon" src="img/admin.png" title="Administrator" />';
			}
			else if(userRole == 17){
				// ChatBot
				var userRoleIcon = '<img alt="" class="rolicon" src="img/bot.png" title="ChatBot" />';
			}
		// HDVinnie User Class Icons
			str	= '<div id="'
	...
	getUserNodeStringItems: function(encodedUserName, userID, isInline) {
		...
		if(encodedUserName !== this.encodedUserName) {
			...
			if(this.userRole === '15' || this.userRole === '16') {
			...
			menu 	= '<li><a href="javascript:ajaxChat.sendMessageWrapper(\'/who\');">'
					...
					+ '</a></li>';
			if(this.userRole === '12' || this.userRole === '13' || this.userRole === '14' || this.userRole === '15' || this.userRole === '16') {
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		// HDVinnie Datetime Clock
		var dateTime = this.settings['dateFormat'] ? '<img class="clockTime" src="img/clock.png" title="'
		...
		return	'<div id="'
				...
				+ ' dir="'
				+ this.baseDirection
				+ '" onclick="ajaxChat.insertText(\'['+userClass+']@\' + this.firstChild.nodeValue + \':[/'+userClass+'] \');">'
				+ userName
				+ '</span>'
				+ colon
	...
	isAllowedToDeleteMessage: function(messageID, userID, userRole, channelID) {
		if((((this.userRole === '1' && this.allowUserMessageDelete && (userID === this.userID ||
			...
			this.userRole === '15') && userRole !== '16') || this.userRole === '16') {
			return true;
		}
		return false;
	},
	...
	insertBBCode: function(bbCode) {
		switch(bbCode) {
			case 'url':
				...
				break;
			case 'youtube':
				var url = prompt(this.lang['ytDialog'], 'http://');
				var ytTitle = this.lang['ytVideo'];
				if(url)
					this.insert('[youtube=' + url + ']', ytTitle + '[/youtube]');
				else
					this.dom['inputField'].focus();
				break;
			default:
	...
	replaceBBCodeCallback: function(str, p1, p2, p3) {
		...
		switch(p1) {
			...
			case 'youtube':
				return ajaxChat.replaceBBCodeYTube(p3, p2);
			default:
	...
	replaceBBCodeYTube: function(content, url) {
		if(!url || url === 'http://')
			url = content;
		var ytLink = this.replaceBBCode(url).replace('http://', '') .replace('https://', '').replace('www.', '').replace('youtu.be/', '').replace('youtube.com/watch?v=', '').split('&', 1).join().replace('>', '*').replace('<', '*');
		if(content)
			var ytTitle = content.replace('http://', '').split('&', 1).join();
		else
			var ytTitle = this.lang['ytVideo'];
		return	'<a href="http://www.youtube.com/embed/'
				...
				+ '" onclick="openWindow(this.href);this.blur();return false;">'
		 		+ '<img src="./img/youtube.png" /></a> <a href="http://www.youtube.com/embed/'
				+ ytLink
		 		+ '" title="'
				+ this.lang['ytTitle']
				+ '" onclick="openWindow(this.href);this.blur();return false;">'
				+ ytTitle
				+ '</a> <a href="http://www.youtube.com/embed/'
				+ ytLink
		 		+ '" title="'
				+ this.lang['ytTitle']
				+ '" onclick="openWindow(this.href);this.blur();return false;">'
