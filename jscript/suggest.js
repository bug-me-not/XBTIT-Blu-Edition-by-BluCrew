...
function suggest(key,query,type) {
	if (key == 38) {
		goPrev();
	} else if (key == 40) {
		goNext();
		...
		if (query.length > 3) {
			t = new Date();
			ajax.get('/suggest.php?a='+type+'&q='+query+'&bla='+t.getTime(),update);
		} else {
			update('');
		}
	}
...
function update(result) {
	...
	if (arr[0].length > 0) {
		...
		suggdiv.innerHTML = '';
		suggdiv.style.height = count * 15;
	
		for (i = 1; i <= count; i++) {
			novo = document.createElement("div");
			suggdiv.appendChild(novo);
			novo.id = i;
			novo.style.width = "332px";
			novo.style.padding = "1px";
			novo.style.paddingRight = "3px";
			novo.style.paddingLeft = "3px";
...
function suggest_users(key,query,type) {
	if (key == 38) {
		goPrev();
	} else if (key == 40) {
		goNext();
		...
		if (query.length > 3) {
			t = new Date();
			ajax.get('/suggest.php?a='+type+'&q='+query+'&bla='+t.getTime(),update_users);
		} else {
			update_users('');
		}
	}
...
function select(obj,mouse) {
	obj.style.backgroundColor = '#075ca0';
...
function unselect(obj,mouse) {
	obj.style.backgroundColor = '#373737';
	obj.style.color = '#ffffff';
	obj.style.fontWeight = 'normal';
	if (mouse) {
		pos = 0;
...
function unselectAllOther(id) {
	for (i = 1; i <= count; i++) {
		if (i != id) {
			document.getElementById(i).style.backgroundColor = '#373737';
